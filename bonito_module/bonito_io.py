...
def write_fastq(header, sequence, qstring, fd=sys.stdout, tags=None, sep="\t",output=None):
    #print("?????????????????????????????-----------------------")
    #sys.stderr.write(f"<batch1> {fd}\n")
    """
...
class Writer(Thread):
    ...
    def __init__(
            ...
            ref_fn=None, groups=None, group_key=None, min_qscore=0,output=None
    ):
        super().__init__()
    ...
    def run(self):
        with CSVLogger(summary_file(), sep='\t') as summary:
            for read, res in self.iterator:
                ...
                if len(seq):
                    if self.mode == 'wfq':
                        write_fastq(read_id, seq, qstring, fd=self.fd, tags=tags,output=self.output_file)
                    # else:
                    #     self.output.write(
