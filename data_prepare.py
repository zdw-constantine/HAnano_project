...
from utils.seeds import DATA_PREPARE_READ_SHUFFLE
...
def segment_read(read_file, window_length, overlap, min_bases, max_bases):
    """将一条信号/fast5处理成相应的numpy向量用于训练 Processes a single/multi fast5 read into chunks for deep learning training
...
def main(fast5_dir, fast5_files_list, output_dir, total_files, 
    ...
    """处理一系列基于fast5文件的reads用于训练 Processes a set of fast5 reads into chunks for deep learning training
    ...
    # 找到所用的我们想要处理的文件 find all the files that we have to process
    files_list = list()
    if fast5_dir is not None:
        print("Find fast5_dir to process , fast5_dir : " + fast5_dir+';')
        for path in Path(fast5_dir).rglob('*.fast5'):
            files_list.append(str(path))
        print("File fast5 num = "+str(len(files_list)))
    elif fast5_files_list is not None:
        with open(fast5_files_list, 'r') as f:
            for line in f:
                line=line.replace('\n','')
                print("Find fast5_dir to process , fast5_dir : " + line+';')
                for path in Path(line).rglob('*.fast5'):
                    files_list.append(str(path))
                print("File fast5 num = "+str(len(files_list)))
    else:
    ...
    #print(DATA_PREPARE_READ_SHUFFLE)
    
    #打乱file_list divide files over total number of files
    random.seed(DATA_PREPARE_READ_SHUFFLE)
    random.shuffle(files_list)
    
    #根据total_files=n，切分数据集为n份
    reads_splits = np.array_split(files_list, total_files)
    #print(reads_splits)
    jobs = list()

    #将reads_splits中切分好的数据一份份的送入到segment_reads_and_write处理
...
if __name__ == "__main__":
    ...
    #fast5-dir:指定fast5文件夹的路径
    #fast5-dir：未知
    #output-dir:输出numpy文件的路径
    #total-files:总共文件
    #windows-size 信号采样窗口，默认2000
    #window-slide 
